...
const Recipes: FC<IRecipesProps> = () => {
  ...
  const RecipeSelectionComponent = () => {
  ...
  const RecipeParametersComponent = () => {
    if (hmi?.recipes[selected]) {
      ...
      return <ul className="flex flex-col gap-4 pt-4 mt-4">{params}</ul>;
    } else {
      return (
        <ul className="flex flex-col gap-4 pt-4 mt-8">Parametry niedostępne</ul>
      );
    }
  };
      ...
      <section className="flex flex-col w-full p-4 mr-4 bg-white shadow-lg rounded-2xl">
        <div className="flex justify-between">
          ...
          <div className="text-2xl">
            ...
            <button className="p-2 mr-4 text-gray-800 bg-gray-200 rounded-lg">
              <BsTrash />
            </button>
            <button className="p-2 text-gray-800 bg-gray-200 rounded-lg">
              <BsPencil />
            </button>
          </div>
        </div>
